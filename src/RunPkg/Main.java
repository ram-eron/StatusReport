...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		while (true) {
			...
			if (diaExecutado != diaAtual) {
				//reinicia variaveis ao trocar o dia
				executeCount = 0; 
				executado = true;
				...
				//verifica se esta na hora programada
			if ( hora == horaDoEnvio && executado == true ) {
				try {
					...
					System.out.println("Loga erro no envio do email " + e);
					
				}finally {
					//controle para tentar novamente em caso de erro
					if (executeCount == 2) {
	...
	private static void Executa() throws Exception {
		...
		dadosReport.forEach((k,v) -> {
					report.Autentica();
					System.out.println(" - Log - " + dadosReport.get(k));
